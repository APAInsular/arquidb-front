                            ...
                            <div className="mt-6">
                                {expedientDocuments.map(({ phase, documents }) => (
                                    <div key={phase.id} className="gap-6">
                                        <p className="text-center text-2xl font-semibold mb-2">Fase {phase.phase}</p>
                                        <div className="gap-4">
                                            {documents.map(document => (
                                                <div key={document.id} className="grid grid-cols-2 py-1.5 px-4 hover:bg-[#bb2b46]/60 hover:text-white even:bg-[#bb2b46]/8 mt-2 cursor-pointer transition-all shrink-0 overflow-x-scroll">
                                                    <Link to={import.meta.env.VITE_APP_BACKEND_URL + "/storage/" + document.name}
                                                        className="">
